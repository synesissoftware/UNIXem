...
namespace {
    ...
    void TEST_readdir_CWD_FOR_FILES_ONLY();
} // anonymous namespace
...
int main(int argc, char** argv)
{
    ...
    if (XTESTS_START_RUNNER("test.component.dirent", verbosity))
    {
        ...
        XTESTS_RUN_CASE(TEST_readdir_CWD_FOR_FILES_ONLY);
...
# pragma warning(pop)
    ...
    else
    {
        ...
        REQUIRE(TEST_INT_EQ(rds_entries.size(), rd_entries.size()));
...
# pragma warning(pop)
    ...
    else
    {
        ...
        for (dirent* de; NULL != (de = ::readdir(dir)); )
        {
            if (fs_traits_t::is_directory(de->d_name))
            {
                continue;
            }
